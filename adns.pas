 ...
 * adns.pas
 * - Pascal conversion of the C header
 *}
{*
 *
 *  This file is
 ...
 *
 *  based on adns.h which is
 *    Copyright (C) 1997-2000,2003,2006,2014 Ian Jackson
 *
 *  which is part of adns, which is
 *    Copyright (C) 1997-2000,2003,2006,2014 Ian Jackson
...
const
 {*adns_initflags - In general, or together the desired flags: *}
 adns_if_none=        $0000;{* no flags.  nicer than 0 for some compilers *}
 adns_if_noenv=       $0001;{* do not look at environment *}
 adns_if_noerrprint=  $0002;{* never print to stderr (_debug overrides) *}
 adns_if_noserverwarn=$0004;{* do not warn to stderr about duff servers etc *}
 adns_if_debug=       $0008;{* enable all output to stderr plus debug msgs *}
 adns_if_logpid=      $0080;{* include pid in diagnostic output *}
 adns_if_noautosys=   $0010;{* do not make syscalls at every opportunity *}
 adns_if_eintr=       $0020;{* allow _wait and _synchronous to return EINTR *}
 adns_if_nosigpipe=   $0040;{* applic has SIGPIPE ignored, do not protect *}
 adns_if_checkc_entex=$0100;{* consistency checks on entry/exit to adns fns *}
 adns_if_checkc_freq= $0300;{* consistency checks very frequently (slow!) *}

 adns_if_permit_ipv4= $0400;{* allow _addr queries to return IPv4 addresses  *}
 adns_if_permit_ipv6= $0800;{* allow _addr queries to return IPv6 addresses *}
 adns_if_afmask=      $0c00;
    ...
    *}
 adns__if_sizeforce= $7fff;

const
 {* adns_queryflags - In general, or together the desired flags: *}
 adns_qf_none=           $00000000;{* no flags *}
 adns_qf_search=         $00000001;{* use the searchlist *}
 adns_qf_usevc=          $00000002;{* use a virtual circuit (TCP conn) *}
 adns_qf_owner=          $00000004;{* fill in the owner field in the answer *}
 adns_qf_quoteok_query=  $00000010;{* allow special chars in query domain *}
 adns_qf_quoteok_cname=  $00000000;{*  ... in CNAME we go via (now default) *}
 adns_qf_quoteok_anshost=$00000040;{*  ... in things supposedly hostnames *}
 adns_qf_quotefail_cname=$00000080;{* refuse if quote-req chars in CNAME we go via *}
 adns_qf_cname_loose=    $00000100;{* allow refs to CNAMEs - without, get _s_cname *}
 adns_qf_cname_strict=   $00010000;{* forbid CNAME refs (default, currently) *}
 adns_qf_cname_forbid=   $00000200;{* don't follow CNAMEs, instead give _s_cname *}

 adns_qf_want_ipv4=	 $00000400;{* try to return IPv4 addresses *}
 adns_qf_want_ipv6=	 $00000800;{* try to return IPv6 addresses *}
 adns_qf_want_allaf=	 $00000c00;{* all the above flag bits *}
    ...
    *}
 adns_qf_ipv6_mapv4=	 $00001000;{*  ... return IPv4 addresses as v6-mapped *}

 adns_qf_addrlit_scope_forbid=$00002000;{* forbid %<scope> in IPv6 literals *}
 adns_qf_addrlit_scope_numeric=$00004000;{* %<scope> may only be numeric *}
 adns_qf_addrlit_ipv4_quadonly=$00008000;{* reject non-dotted-quad ipv4 *}

 adns__qf_internalmask=  $0ff00000;
 adns__qf_sizeforce=     $7fffffff;

const
 {* adns_rrtype *}
 adns_rrt_typemask=  $0ffff;
 adns_rrt_reprmask= $ffffff;
 adns__qtf_deref_bit=$10000;{* internal version of ..._deref below *}
 adns__qtf_mail822=  $20000;{* return mailboxes in RFC822 rcpt field fmt   *}

 adns__qtf_bigaddr=$1000000;{* use the new larger sockaddr union *}
 adns__qtf_manyaf= $2000000;{* permitted to return multiple address families *}

 {* dereference domains; perhaps get extra data *}
 adns__qtf_deref=    adns__qtf_deref_bit or adns__qtf_bigaddr or adns__qtf_manyaf;

 adns_r_unknown=     $40000;
 ...
 adns_r_ns_raw=           2;
 adns_r_ns=                  adns_r_ns_raw or adns__qtf_deref;

 adns_r_cname=            5;

 adns_r_soa_raw=          6;
 adns_r_soa=                 adns_r_soa_raw or adns__qtf_mail822;

 adns_r_ptr_raw=         12; {* do not mind PTR with wrong or missing addr *}
 adns_r_ptr=                 adns_r_ptr_raw or adns__qtf_deref;

 adns_r_hinfo=           13;

 adns_r_mx_raw=          15;
 adns_r_mx=                  adns_r_mx_raw or adns__qtf_deref;

 adns_r_txt=             16;

 adns_r_rp_raw=          17;
 adns_r_rp=                  adns_r_rp_raw or adns__qtf_mail822;
 ...
 adns_r_srv_raw=         33;
 adns_r_srv=                 adns_r_srv_raw or adns__qtf_deref;

 adns_r_addr=                adns_r_a or adns__qtf_deref;

 adns__rrt_sizeforce= $7fffffff;
...
type
  adns_sockaddr_v4only = record
  case integer of
    0: (sa: sockaddr);
    1: (inet: sockaddr_in);
  end;

  adns_sockaddr = record
  case integer of
    0: (sa: sockaddr);
    1: (inet: sockaddr_in);
    2: (inet6: sockaddr_in6);
  end;
  ...
  adns_rr_addr_v4only = record
  {* the old v4-only structure; handy if you have complicated binary
   * compatibility problems. *}
    len: cint;
    addr: adns_sockaddr_v4only;
  end;

  adns_rr_hostaddr = record
    host: PChar;
    astatus: adns_status;
    naddrs: cint; {* temp fail => -1, perm fail => 0, s_ok => >0 *}
    addrs: ^adns_rr_addr;
  end;

  adns_rr_strpair = record
    _array: array[0..1] of PChar;
  end;

  adns_rr_inthostaddr = record
    i: cint;
    ha: adns_rr_hostaddr;
  end;

  adns_rr_intstr = record
  ...
  Padns_rr_intstr = ^adns_rr_intstr;
  PPadns_rr_intstr =^Padns_rr_intstr;

  adns_rr_intstrpair = record
    _array: array[0..1] of adns_rr_intstr;
  end;

  adns_rr_soa = record
    mname, rname: PChar;
    serial, refresh, retry, expire, minimum: culong;
  end;

  adns_rr_srvraw = record
    priority, weight, port: cint;
    host: PChar;
  end;

  adns_rr_srvha = record
    priority, weight, port: cint;
    ha: adns_rr_hostaddr;
  end;

  adns_rr_byteblock = record
    len: cint;
    data: PByte;
  end;

  adns_rrs_union = record
  case integer of
    0: (untyped: Pointer);
    1: (bytes: PByte);
    2: (str: PPChar);                    {* ns_raw, cname, ptr, ptr_raw *}
    3: (manyistr: PPadns_rr_intstr);      {* txt (list strs ends with i=-1, str=0)*}
    4: (addr: ^adns_rr_addr);              {* addr *}
    5: (inaddr: Pin_addr);          {* a *}
    6: (in6addr: Pin6_addr);        {* aaaa *}
    7: (hostaddr: ^adns_rr_hostaddr);      {* ns *}
    8: (intstrpair: ^adns_rr_intstrpair);  {* hinfo *}
    9: (strpair: ^adns_rr_strpair);        {* rp, rp_raw *}
    10:(inthostaddr: ^adns_rr_inthostaddr);{* mx *}
    11:(intstr: ^adns_rr_intstr);          {* mx_raw *}
    12:(soa: ^adns_rr_soa);                {* soa, soa_raw *}
    13:(srvraw: ^adns_rr_srvraw);          {* srv_raw *}
    14:(srvha: ^adns_rr_srvha);{* srv *}
    15:(byteblock: ^adns_rr_byteblock);    {* ...|unknown *}
  end;

  adns_answer = record
    status: adns_status;
    cname: PChar; {* always NULL if query was for CNAME records *}
    owner: PChar; {* only set if req'd in query flags; maybe 0 on error anyway *}
    _type: cint; {* adns_rrtype - guaranteed to be same as in query *}
    expires: time_t;{*abs time.  def only if _s_ok, nxdomain or nodata. NOT TTL!*}
    nrrs, rrsz: cint; {* nrrs is 0 if an error occurs *}
    rrs: adns_rrs_union;
  end;
  Padns_answer = ^adns_answer;
  PPadns_answer = ^Padns_answer;
...
function adns_init(var newstate_r: adns_state; flags: cint{*adns_initflags*};
              diagfile: Pointer{*0=>stderr*}): cint; cdecl; external adnslib;

function adns_init_strcfg(var newstate_r: adns_state; flags: cint{*adns_initflags*};
              diagfile: Pointer {*0=>discard*}; const configtext: PChar): cint; cdecl; external adnslib;
(*
...
function adns_synchronous(ads: adns_state;
                          const owner: PChar;
                          _type: cint; {adns_rrtype}
                          flags: cint; {adns_queryflags}
                          var answer: Padns_answer): cint; cdecl; external adnslib;
 ...
 *}
function adns_submit(ads: adns_state;
                     const owner: PChar;
                     _type: cint; {adns_rrtype}
                     flags: cint; {adns_queryflags}
                     context: Pointer;
                     var query_r: adns_query): cint; cdecl; external adnslib;

{* The owner should be quoted in master file format. *}

function adns_check(ads: adns_state;
                    var query_io: adns_query;
                    var answer_r: Padns_answer;
                    context_r: PPointer): cint; cdecl; external adnslib;

function adns_wait(ads: adns_state;
                   var query_io: adns_query;
                   var answer_r: Padns_answer;
                   context_r: PPointer): cint; cdecl; external adnslib;

{* same as adns_wait but uses poll(2) internally *}
function adns_wait_poll(ads: adns_state;
                        var query_io: adns_query;
                        var answer_r: Padns_answer;
                        context_r: PPointer): cint; cdecl; external adnslib;

procedure adns_cancel(query: adns_query); cdecl; external adnslib;
...
function adns_submit_reverse(ads: adns_state;
...
function adns_submit_reverse_any(ads: adns_state;
                        const addr: Psockaddr;
                        const rzone: PChar;
                        _type: cint; {adns_rrtype}
                        flags: cint; {adns_queryflags}
...
procedure adns_finish(ads: adns_state); cdecl; external adnslib;
...
const
  INET6_ADDRSTRLEN = 48;
  IF_NAMESIZE = 16;       { true for Linux... }
{$ENDIF}
const
  ADNS_ADDR2TEXT_BUFLEN = (INET6_ADDRSTRLEN + 1{*%*} + IF_NAMESIZE);
(*
...
*)
function adns_text2addr(const _text: PChar; port: cuint16; flags: cint {adns_queryflags};
                   var sa_r: sockaddr;
                   var salen_io: socklen_t {* updated iff OK or ENOSPC *}): cint; cdecl; external adnslib;
function adns_addr2text(const sa: sockaddr; flags: cint; {adns_queryflags}
                   buffer: PChar; buflen_io: Pcint; {* updated ONLY on ENOSPC *}
                   port_r: Pcint {* may be 0 *}): cint; cdecl; external adnslib;
  {*
...
procedure adns_forallqueries_begin(ads: adns_state); cdecl; external adnslib;
function adns_forallqueries_next(ads: adns_state; context_r: PPointer): adns_query; cdecl; external adnslib;
...
procedure adns_checkconsistency(ads: adns_state; qu: adns_query); cdecl; external adnslib;
...
function adns_processany(ads: adns_state): cint; cdecl; external adnslib;
...
function adns_processreadable(ads: adns_state; fd: cint; const now: Ptimeval): cint; cdecl; external adnslib;
function adns_processwriteable(ads: adns_state; fd: cint; const now: Ptimeval): cint; cdecl; external adnslib;
function adns_processexceptional(ads: adns_state; fd: cint; const now: Ptimeval): cint; cdecl; external adnslib;
...
procedure adns_processtimeouts(ads: adns_state; const now: Ptimeval); cdecl; external adnslib;
...
procedure adns_firsttimeout(ads: adns_state;
                       var tv_mod: Ptimeval; tv_buf: Ptimeval;
                       now: timeval); cdecl; external adnslib;
...
procedure adns_globalsystemfailure(ads: adns_state); cdecl; external adnslib;
...
procedure adns_beforeselect(ads: adns_state; maxfd: Pcint; var readfds: fd_set;
                       var writefds: fd_set; var exceptfds: fd_set;
                       tv_mod: PPtimeval; tv_buf: Ptimeval;
                       const now: PTimeval); cdecl; external adnslib;
...
procedure adns_afterselect(ads: adns_state; maxfd: cint; var readfds: fd_set;
                           var writefds: fd_set; var exceptfds: fd_set;
                           const now: Ptimeval); cdecl; external adnslib;
...
function adns_beforepoll(ads: adns_state; var fds: pollfd;
                    nfds_io: Pcint; timeout_io: Pcint;
                    const now: Ptimeval): cint; cdecl; external adnslib;
...
procedure adns_afterpoll(ads: adns_state; var fds: pollfd; nfds: cint;
                         const now: Ptimeval); cdecl; external adnslib;
{* Gives adns flow-of-control for a bit; intended for use after
...
function adns_rr_info(_type: cint {adns_rrtype};
                      rrtname_r: PPchar; fmtname_r: PPchar;
                      len_r: pcint;
                      const datap: Pointer; data_r: PPchar): adns_status; cdecl; external adnslib;
{*
...
function adns_strerror(st: adns_status): PChar; cdecl; external adnslib;
function adns_errabbrev(st: adns_status): PChar; cdecl external adnslib;
function adns_errtypeabbrev(st: adns_status): PChar; cdecl external adnslib;
